...
knn_pred <- knn_learner$train(spam_task)$predict(spam_task)
...
# Threshold of 60% probability of spam:
knn_pred$set_threshold(0.6)
knn_pred$confusion
knn_pred$score(measures)
...
For an alternative that considers varying thresholds, we can use 
[ROC curves](https://en.wikipedia.org/wiki/Receiver_operating_characteristic).

To do so, we first have to adjust our Learner to return the spam *probability*
...
we overfit in some way or the data is weird (technical term).

To get the AUC we use `msr("classif.auc")` instead of `"classif.acc"` going forward:

```{r knn-auc}
knn_pred$score(msr("classif.auc"))
```


## Your Turn!

1. Repeat the same steps (train learner, get AUC) for the `{rpart}` decision tree learner.
  - Does it fare better than kNN with default parameters?
2. Repeat either learner resampling with different hyperparameters. 
  - (Don't worry, in the next section we're doing that less manually)
...
# Extract per-iteration accuracy per learner (only first five rows shown)
...
Can our fancy ML methods beat the good old GLM?
