...
We saw that `k` is an important parameter, and it's an integer greater than 1 at least. To tune it, we also have to make a few other decisions, also using what we learned about resampling.
...
We'll use `{mlr3}`'s [`auto_tuner`](https://mlr3book.mlr-org.com/chapters/chapter4/hyperparameter_optimization.html#sec-autotuner) for this because it's just so convenient:
...
In this case, we get a `k` of 25, with an AUC of about 0.96 in the inner resampling (the holdout set during tuning).

We can visualize the performance across all the `k`s we tried by accessing the tuning instance now included in the `tuned_knn` learner object:

```{r plot-knn-tuning-result}
autoplot(tuned_knn$tuning_instance, type = "performance")
...
tuned_knn$tuning_instance$result_learner_param_vals
```

Now that we've tuned on the training set, it's time to evaluate on the test set, and to shorten it a little we'll do prediction and scoring in one line:

```{r knn-eval}
tuned_knn$predict(spam_task, row_ids = spam_split$test)$score(msr("classif.auc"))
```
...
# Get AUC for outer folds
knn_nested_tune$score(msr("classif.auc"))[, .(iteration, classif.auc)]

# Average over outer folds - our "final result" performance for kNN
# AUC is averaged over outer folds
knn_nested_tune$aggregate(msr("classif.auc"))
```
