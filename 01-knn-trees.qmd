...
Proportion of correct predictions ("accuracy") and **incorrect** predictions (classification error):
...
(R shortcut: Logical comparison gives logical vector of `TRUE`/`FALSE`, which is used like 1/0 for math, so we can sum it up (== correct classifications) and divide by N for the proportion, i.e. calculate the mean)
...
Identical methods to set multiple or a single hyperparam:
