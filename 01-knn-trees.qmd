...
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g, fill = species)) +
  ...
  geom_point(shape = 21, stroke = 1/4, size = 3, alpha = 2/3, key_glyph = "rect") +
  labs(
...
{{< include solutions/01-rpart.qmd >}}
...
ggplot(pred_grid, aes(x = flipper_length_mm, y = body_mass_g)) +
  ...
  geom_point(
    data = penguins_test, aes(fill = species), 
    shape = 21, 
    color = "black", 
    size = 2,
    key_glyph = "rect"
  ...
  labs(
    ...
    subtitle = paste("Species as predicted by decision tree", 
    "Point color is the true species", sep = "\n"),
    x = "Flipper Length [mm]", y = "Body Mass [g]",
    fill = "Species"
  ) +
  theme_minimal(base_size = 13) +
