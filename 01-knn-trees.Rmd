...
Now that we've done the kNN fitting with `{mlr3}`, you can easily do the same thing with the `rpart`-learner! All you have to do is switch the learner objects.

Here's the same code as above but compacted for you to play around with, just make it do the same steps (training, predicting) with the `rpart_learner`.
