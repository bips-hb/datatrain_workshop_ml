...
See [their website](https://allisonhorst.github.io/palmerpenguins/) for some more information if your interested. For now it's enough to know that we have a bunch of data about 3 species of penguins.
...
Now that we have some data, we can start fitting models, just to see how it goes! Given the plot from earlier, we may have a rough idea that the flipper length and body mass measurements are already giving us a somewhat decent good picture about the species.
...
Can you make it even better? Can you change something to make it *worse*?
...
Now that we've played around with kNN a little, let's grow some trees! We'll use the `{rpart}` (**R**ecursive **Part**itioning) package and start with the same model specification as before and use the default parameters.
...
ggplot(pred_grid, aes(x = flipper_length_mm, y = body_mass_g, color = rpart_prediction)) +
  ...
  geom_point(data = penguins_train, aes(fill = species), shape = 21, color = "black") +
  labs(
    title = "Palmer Penguins",
...
Now, imagine you want to try out some more hyperparameters for either `rpart()` or `kknn()` or both, and then you want to compare the two - that would probably be kind of tedious unless to write some wrapper functions, right? Well, luckily we're not the first people to do some machine learning!

Our code above works (hopefully), but for any given model or algorithm, there's different R packages with slightly different interfaces as you've seen. `{mlr3}` and add-on packages unify all the common tasks with a consistent interface.
...
# We can also explicitly assign the feature colums
...
We're mixing things up with a new train/test split, just for completeness in the example. For mlr3, we only need to save the indices.
...
# Or more specifically, the kknn OR rpart learners
...
To calculate the prediction accuracy, we don't have to do any math in small steps. `{mlr3}` comes with lots of measures (like accuracy) we can use, they're organized in the `mlr_measures` object (just like `mlr_learners`). We're using `"classif.acc"` here with the shorthand function `msr()`. The we score our predictions with this measure, using the `$score()` method.
...
As a bonus feature, mlr3 also makes it easy for us to plot the decision boundaries
for a two-predictor case, so we don't have to manually predict on a grid anymore.  
...
# Train
rpart_learner$train(penguin_task, row_ids = penguin_train)

# Predict
rpart_prediction <- rpart_learner$predict(penguin_task, row_ids = penguin_test)
