...
Given the plot from earlier, we may have a rough idea that the flipper length and body mass measurements are already giving us a somewhat decent good picture about the species.
...
rpart_penguins <- rpart(
  ...
  maxdepth = 30  # Default (and upper limit!) = 30
)
```
...
2.  Try out trees with different sizes (`maxdepth`) and other parameters, including more predictors (`formula =`)
...
Now, imagine you want to try out some more hyperparameters for either `rpart()` or `kknn()` or both, and then you want to compare the two - that would probably be kind of tedious unless to write some wrapper functions, right?
Well, luckily we're not the first people to do some machine learning!

Our code above works (hopefully), but for any given model or algorithm, there's different R packages with slightly different interfaces as you've seen.
`{mlr3}` and add-on packages unify all the common tasks with a consistent interface.

## Creating a task

The task encapsulates our data, including which variables we're using to learn and which we want to predict.
Tasks can be created in [multiple ways and some standard example tasks are available in mlr3](https://mlr3book.mlr-org.com/tasks.html), but we're taking the long way around.
...
-   What are we predicting on?
    ...
    -   The dataset, specified as `backend =`
...
# For the kknn OR rpart learners
...
`{mlr3}` comes with lots of measures (like accuracy) we can use, they're organized in the `mlr_measures` object (just like `mlr_learners`).
...
(You can ignore any warnings from the plot here)
