...
This works by creating a _pipeline_ using the `%>>%` oeprator (not to be confused with the magrittr-pipe `%>%` you might be familiar with!).
We take the `encode` pipeline operation (`PipeOp`) and stack it on top of our learner we create as usual.
At the end we convert it to a regular learner, and `lrn_svm` is now a regular mlr3 learner we can use like any other, but with built-in encoding!
